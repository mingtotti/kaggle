...
Npp8u **
decode_image(const char *inputFile, int &inputStep, NppiSize &inputSize, bool &gray) {
  ...
  for (int set = 0; set < 10; set++) {
    for (int subSet = 0; subSet < 10; subSet++) {
      ...
      for (int imageID = set * 10 + subSet; imageID < 14666499; imageID += 100) {
        ...
        NppiSize inputSize;
        bool gray = false;
        Npp8u **pInputImage = decode_image(szInputFile, inputStep, inputSize, gray);
        if (pInputImage == NULL) {
          if (gray) {
        ...
        for (int row = 0; row < resizeSize.height; row++) {
          for (int col = 0; col < resizeSize.width - 1; col++) {
            unsigned char pixel_left = pHost[row * resizeSize.width + col];
            unsigned char pixel_right = pHost[row * resizeSize.width + col + 1];
